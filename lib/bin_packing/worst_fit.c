...
unsigned int worst_fit_allocation(taskset *ts, cpu* c, FILE * f) {
	unsigned int i, j = 0;
	
	fprintf(f, "Let's try to assign the tasks to the different cores according to the worst fit heuristic algorithm:\n");
	for (i = 0; i < ts->size; i++) {
		sort_by_decreasing_empty_space(c);
		fprintf(f, "Task %d :\n", ts->tasks[i].id);
