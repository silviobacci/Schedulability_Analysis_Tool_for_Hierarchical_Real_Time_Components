...
static void h_print(taskset *ts, unsigned int Qs, unsigned int Ts){
	...
	if (n_sp > 0) {
		...
		for (j = 0; j < n_sp; j++) {
			printf("\t dbf(%d) = %f; sbf(%d) = %u\n", sp[j], dbf_edf(ts, sp[j]), sp[j], sbf(Qs, Ts, sp[j]));
		
			if (dbf_edf(ts, sp[j]) > sbf(Qs, Ts, sp[j])) 
				schedulable = 0;
		}

		if(schedulable)
			printf("The task set is schedulable under EDF with the specified periodic server.\n");
		else
			printf("The task set is NOT schedulable under EDF with the specified periodic server.\n");
	}
}
